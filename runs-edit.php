...
if( isset($_POST['Edit']) ) {
	$conn = connect_db();
		...
		$sql = "UPDATE runs SET name='". $_POST['name'] ."', note='". htmlspecialchars($_POST['note']) ."' WHERE id_run=". $_POST['id'];
...
if ($res) {
	...
	text: "Run Type has been edited without any problem!",
	type: "success",
...
$conn = connect_db();
	...
	$sql = "SELECT * FROM runs WHERE id_run=". $id;
	$res = mysqli_query($conn, $sql);
